...
| `.agents/notes/` | **本仓库作为软件项目的工程决策**——ponygo 这个软件本身怎么实现（"这个项目自己怎么被治理"） | CLI 用单文件 bash；测试用 mktemp 场景化；某 bug 的修复方案 |

实例项目只有 `.agents/notes/`（revisions/ 不进实例骨架）；框架维护者两套都用。
